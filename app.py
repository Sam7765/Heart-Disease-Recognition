...
# Updated line to use the correct method
st.set_page_config(page_title="Heart Failure Prediction", page_icon="💉", layout='centered', initial_sidebar_state='auto')
...
if st.button("Predict"):
    ...
    for live, die in loaded_model.predict_proba(single_sample):
        live = f"{live*100:.2f}%"
        die = f"{die*100:.2f}%"
        st.table(pd.DataFrame({'Live ': live, 'Die': die}, index=['probability']))
